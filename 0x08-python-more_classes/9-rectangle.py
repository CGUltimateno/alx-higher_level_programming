...
class Rectangle:
    ...
    def bigger_or_equal(rect_1, rect_2):
        """Return the silly rectangle with more area"""
    ...
    def square(cls, size=0):
        """returns a silly rectangle with equal width, height, and size"""
        return cls(size, size)

    def __repr__(self):
        """Return string rep of the silly rectangle"""
        rect = "Rectangle(" + str(self.__width)
        rect += ", " + str(self.__height) + ")"
        return rect

    def __del__(self):
        """Return byebye silly rectangle"""
        type(self).number_of_instances -= 1
        print("Bye rectangle...")
